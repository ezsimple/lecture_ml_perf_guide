...
export_graphviz(dt_clf, out_file="data/tree.dot", class_names=iris_data.target_names , \
...
# 분할을 많이 할 수록 feature importance가 높아짐.
# Tree 계열에는 feature_importance_ 가 모두 있습니다.
# Feature 수만큼 표시합니다.
...
# refit=True 옵션을 주면 가장 최고의 평균 정확도를 가진 하이퍼 파라미터를 사용하여 최종 학습
grid_cv = GridSearchCV(dt_clf, param_grid=params, scoring='accuracy', cv=5, verbose=1 )
grid_cv.fit(X_train , y_train)
print('GridSearchCV 최고 평균 정확도 수치: {0:.4f}'.format(grid_cv.best_score_))
print('GridSearchCV 최적 하이퍼 파라미터:', grid_cv.best_params_)
